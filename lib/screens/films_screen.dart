...
class _FilmsScreenState extends State<FilmsScreen> {
  ...
  fetchCategories() async {
    var response = await http.get(Uri.parse('https://api.themoviedb.org/3/genre/movie/list?api_key=$apiKey&language=en-US'));
  ...
  fetchMoviesForCategory(int categoryId) async {
    var response = await http.get(Uri.parse('https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&with_genres=$categoryId'));
    if (response.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Center(child: Text('Films Categories')),
